        ...
        <div className="text-slate-500 mb-40">
            ...
            <form onSubmit={(e) => toast.promise(handleAddCoupon(e), { loading: 'Adding coupon...' })} className="max-w-sm text-sm">
                ...
                <div className="flex gap-2 max-sm:flex-col mt-2">
                    <input type="text" placeholder="Coupon Code" name="code" value={newCoupon.code} onChange={handleChange} required className="border border-slate-200 w-full mt-2 p-2 outline-slate-800 rounded-md" />
                    <input type="number" placeholder="Coupon Discount (%)" name="discount" value={newCoupon.discount} onChange={handleChange} required className="border border-slate-200 w-full mt-2 p-2 outline-slate-800 rounded-md" />
                </div>
                <input type="text" placeholder="Coupon Description" name="description" value={newCoupon.description} onChange={handleChange} className="border border-slate-200 w-full mt-2 p-2 outline-slate-800 rounded-md" required />

                <label>
                    <p>Coupon Expiry Date</p>
                    <input type="date" placeholder="Coupon Expire At" name="expiresAt" value={format(newCoupon.expiresAt, 'yyyy-MM-dd')} onChange={handleChange} className="border border-slate-200 w-full mt-2 p-2 outline-slate-800 rounded-md" />
                </label>
                ...
                <div className="mt-5">
                 ...
                 <button className="bg-slate-700 text-white mt-4 p-2 px-10 rounded active:scale-95 transition">Add Coupon</button>
            </form>
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {coupons.map((coupon)=>(
                                <tr key={coupon.code} className="hover:bg-slate-50">
                                   ...
                                   <td className="py-3 px-4  text-slate-800">{format(coupon.expiresAt,'yyy-MM-dd')}</td>
